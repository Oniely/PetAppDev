				...
				<div className="flex flex-col justify-between sm:ml-12 max-sm:mt-4 h-[300px] max-sm:h-auto max-sm:gap-4">
					<div className="flex flex-col gap-1">
						<div className="flex items-center gap-2 mb-2 border-b border-b-neutral-500">
							<p className="text-xl font-semibold">{service.serviceName}</p>
							<p className={`text-neutral-700 relative after:content-[''] after:w-2 after:h-2 after:absolute after:top-0.5 after:ml-0.5 after:animate-pulse after:rounded-full ${service.status ? 'after:bg-green-400' : 'after:bg-yellow-400'}`}>- {service.status ? "Available" : "Not Available"}</p>
						</div>
						<p className="text-neutral-800 lowercase">
							<span className="capitalize">{service.typeOfService.split('_').join(' ')}</span>
						</p>
						<p className="text-neutral-700 font-light">Duration: <span className="font-medium">{service.duration}mins</span></p>
						<p className="text-neutral-700 font-light">Price: <span className="font-medium">₱{service.price}</span></p>
						<ScrollArea className="max-h-[120px]">
							<p className="text-neutral-700 font-light text-sm">{service.description}</p>
						</ScrollArea>
					</div>
					<div className="flex items-center gap-3">
						<Button className="flex-1">See Appointments</Button>
